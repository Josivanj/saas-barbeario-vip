...
function vipInstagramUrl(value='') {
 const profile=String(value).trim();
 if(!profile)return '';
 if(/^https?:\/\/(www\.)?instagram\.com\//i.test(profile))return profile;
 return `https://instagram.com/${encodeURIComponent(profile.replace(/^@/,''))}`;
}
...
async function renderPublicContact(){
 ...
 if(instagramLink&&instagram)instagramLink.href=vipInstagramUrl(instagram);
}
